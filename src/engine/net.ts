/**
 * net.ts — zero-backend P2P networking (copied from patterns/, unmodified API).
 *
 * Thin wrapper over Trystero: an encrypted WebRTC mesh over free public Nostr
 * relays — no server of ours, which is exactly what GitHub Pages hosting needs.
 * Netcode model: host-authoritative star. Every peer runs the same election
 * (lexicographically smallest peer id) so they all agree on the host with no
 * handshake, and it re-elects automatically when the host leaves.
 *
 * Channels must be <= 12 bytes. Payloads are JSON-serialized; keep small.
 */
