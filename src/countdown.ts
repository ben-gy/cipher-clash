/**
 * countdown.ts — the three seconds between "everyone's ready" and the first tile.
 *
 * Two jobs. The obvious one is fairness: a word grid starts the instant it is
 * visible, so without a beat to look up, whoever happened to be staring at the
 * screen when the round fired gets a free head start. The quieter one is that it
 * tells you the round is *about* to be yours — a board that simply appears reads
 * as a jump-cut.
 *
 * The audio matters more than the number. Players look at the grid, not the
 ...
 * host-authoritative anyway, so that skew costs nobody a word.
 */
